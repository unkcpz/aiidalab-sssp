"""Widget for verification calculation"""
import re

from aiida.plugins import WorkflowFactory
from aiida.engine import submit
from aiida.orm.nodes.data import Bool, Dict, List, Str, UpfData
...
class ComputeVerificationWidget(ipw.VBox):
    ...
    def __init__(self, **kwargs):
        ...
        pw_code = CodeDropdown(
            input_plugin="quantumespresso.pw",
            description="Pw code",
            setup_code_params=setup_pw_code_params,
        )
        ph_code = CodeDropdown(
            input_plugin="quantumespresso.ph",
            description="Ph code",
...
protocol.yml
        ...
        self.dual = ipw.BoundedIntText(
            value=8, step=1, min=1, description="# dual", **extra
        )

        # set the parameters for delta factor calculation
        self.parameters = ipw.HBox(
            children=[
                ipw.VBox(
                    children=[
    ...
    def submit(self):
        ...
        if self.input_pseudo is None:
            ...
            builder.protocol = Str(self.protocol.value)
            ...
            ecutrho = [_ * self.dual.value for _ in ecutwfc]
            builder.parameters.ecutwfc_list = List(ecutwfc)
            builder.parameters.ecutrho_list = List(ecutrho)

            builder.options = Dict(self.options)
